      ...
      switch (mode.value) {
        ...
        case 4:
          if (randomSource.value === 'sina') {
            output = `#242428 url(http://139.199.175.149/kf_api/api/randomPhoto/sina?duration=${duration.value}) center center / cover`
          } else if (randomSource.value === 'bing') {
            output = `#242428 url(http://139.199.175.149/kf_api/api/randomPhoto/bing?duration=${duration.value}) center center / cover`
          } else if (randomSource.value === 'personal') {
            output = `#242428 url(http://139.199.175.149/kf_api/api/randomPhoto/personal?duration=${duration.value}) center center / cover`
          } else {
            const keyword = imgType.value === 'Custom' ? customImgType.value : imgType.value
            const mirrorStr = mirror.value ? '&type=mirror' : ''
            output = `#242428 url(http://139.199.175.149/kf_api/api/randomPhoto?keyword=${keyword}&w=${w.value}&h=${h.value}${mirrorStr}&duration=${duration.value}) center center / cover`
          }
          break
        case 5:
